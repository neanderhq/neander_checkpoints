...
# and ignores internal operations (checkpoint commits, etc.)
#
...
#
# Since detect_commit.sh calls checkpoint.sh, we mock it to record calls.
...
SHA_FILE="$REPO_DIR/.git/neander-last-checkpoint-sha"

# detect_commit.sh writes the SHA file synchronously when it triggers.
# We check for SHA file changes instead of checkpoint.sh calls (which are
# backgrounded and disowned).
run_detect() {
    local command="$1"
    local session_id="${2-test-session-id}"
    # Record SHA file state before
    local sha_before=""
    [ -f "$SHA_FILE" ] && sha_before="$(cat "$SHA_FILE")"
    python3 -c "
import json, sys
...
" "$command" "$session_id" "$TRANSCRIPT" | bash "$MOCK_DETECT" 2>/dev/null || true
    ...
    # Check if SHA file was written/changed
    local sha_after=""
    [ -f "$SHA_FILE" ] && sha_after="$(cat "$SHA_FILE")"
...
echo "$OUTPUT" | grep -q "TRIGGERED" && pass "triggers on git commit" || fail "should trigger on git commit"

# --- Test 2: Does NOT trigger on non-commit commands ---
OUTPUT="$(run_detect 'git status')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "should not trigger on git status" || pass "ignores git status"

OUTPUT="$(run_detect 'git push origin main')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "should not trigger on git push" || pass "ignores git push"

OUTPUT="$(run_detect 'git log --oneline')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "should not trigger on git log" || pass "ignores git log"

# --- Test 3: Does NOT trigger on checkpoint internal commands ---
OUTPUT="$(run_detect 'bash checkpoint.sh /tmp/session.jsonl')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "should not trigger on checkpoint.sh" || pass "ignores checkpoint.sh commands"

OUTPUT="$(run_detect 'bash save_summary.sh abc123')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "should not trigger on save_summary.sh" || pass "ignores save_summary.sh commands"

OUTPUT="$(run_detect 'bash persist_summary.sh abc123')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "should not trigger on persist_summary.sh" || pass "ignores persist_summary.sh commands"

# --- Test 4: Does NOT trigger without session ID ---
OUTPUT="$(run_detect 'git commit -m "no session"' '')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "should not trigger without session_id" || pass "ignores empty session_id"
...
echo "$OUTPUT" | grep -q "TRIGGERED" && pass "triggers on user git commit --amend" || fail "should trigger on git commit --amend"

# --- Test 6: Triggers on compound commands containing git commit ---
rm -f "$REPO_DIR/.git/neander-last-checkpoint-sha"
OUTPUT="$(run_detect 'git add . && git commit -m "msg"')"
echo "$OUTPUT" | grep -q "TRIGGERED" && pass "triggers on compound git add && git commit" || fail "should trigger on compound command"
...
echo "$OUTPUT" | grep -q "TRIGGERED" && pass "first commit triggers" || fail "first commit should trigger"

# Second call with same HEAD should be skipped
OUTPUT="$(run_detect 'git commit -m "duplicate"')"
echo "$OUTPUT" | grep -q "TRIGGERED" && fail "duplicate commit should not trigger" || pass "skips duplicate commit at same HEAD"
...
echo "$OUTPUT" | grep -q "TRIGGERED" && pass "new commit after HEAD change triggers" || fail "should trigger after HEAD change"
